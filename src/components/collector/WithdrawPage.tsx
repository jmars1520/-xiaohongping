...
const bankAccounts = [
  { id: 1, bank: "建设银行", number: "****5678", type: "对私", isDefault: true },
  { id: 2, bank: "农业银行", number: "****9012", type: "对私", isDefault: false },
];

const quickAmounts = [100, 300, 500, 1000, 1305];
...
export default function WithdrawPage({ onBack }: WithdrawPageProps) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-8">
        ...
        <div className="text-sm text-gray-500 text-center mb-2">
          提现金额：<span className="text-blue-600 font-bold">¥{Number(amount).toLocaleString()}</span>
        </div>
        ...
        <button
          onClick={onBack}
          className="w-full bg-blue-600 text-white py-3.5 rounded-xl font-semibold text-base shadow-lg"
        >
          返回我的收入
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      {/* Balance Card */}
      <div className="mx-4 mt-4 bg-gradient-to-br from-blue-600 to-blue-700 rounded-2xl p-5 text-white">
        <div className="text-sm text-blue-200">可提现余额（元）</div>
        <div className="text-3xl font-bold mt-1">¥{balance.toLocaleString()}</div>
        <div className="text-xs text-blue-200 mt-1">待结算 ¥255 · 已结算 ¥1,050</div>
      </div>
      ...
      <div className="mx-4 mt-4 bg-white rounded-2xl p-4 shadow-sm">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="请输入提现金额"
            className="w-full border-2 border-gray-200 rounded-xl pl-12 pr-4 py-4 text-2xl font-bold text-gray-800 focus:outline-none focus:border-blue-400 placeholder:text-base placeholder:font-normal placeholder:text-gray-300"
            min={0}
            max={balance}
          />
        </div>
        <div className="flex items-center justify-between mt-2">
          ...
          <button onClick={handleWithdrawAll} className="text-xs text-blue-600 font-medium">
            全部提现
          </button>
        </div>
